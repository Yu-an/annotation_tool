...
class simpleapp_tk(tk.Tk):
  ...
  #arrange things; how
    ...
    def initialize(self):
        self.grid()
        #initialize
        if self.result_df["SpeechAct"][self.index] != None and self.result_df["ClauseType"][self.index] != None:
            self.clausetype= tk.StringVar()
            self.clausetype.set(self.result_df["SpeechAct"][self.index])
            self.speechact = tk.StringVar()
            self.speechact.set(self.result_df["ClauseType"][self.index])
    ...
    def OnButtonClick(self,annot):
        ...
        if self.result_df["SpeechAct"][self.index] != None and self.result_df["ClauseType"][self.index] != None:
            tk.messagebox.showwarning("Warning", "Are you sure you want to change the annotation?")
